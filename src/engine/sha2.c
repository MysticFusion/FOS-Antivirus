 ...
 *
 * Implements the FIPS 180-2 SHA-2 family. Optimized for performance with
...
/* ============================================================================
 * Internal S-Box and Constant Tables
 * ========================================================================== */
...
{
    for (int i = 0; i < 8; i++) ctx->h[i] = sha224_h0[i];
    ctx->len = 0; ctx->tot_len = 0;
}

void sha224_update(sha224_ctx *ctx, const uint8_t *message, uint64_t len)
{
    uint64_t block_nb, new_len, rem_len, tmp_len;
...
{
    uint64_t tot_len = ctx->tot_len + ctx->len;
    uint64_t block_nb = (1 + ((SHA224_BLOCK_SIZE - 9) < (ctx->len % SHA224_BLOCK_SIZE)));
    uint64_t pm_len = block_nb << 6;

    memset(ctx->block + ctx->len, 0, pm_len - ctx->len);
    ctx->block[ctx->len] = 0x80;
    UNPACK64(tot_len << 3, ctx->block + pm_len - 8);

    sha256_transf(ctx, ctx->block, block_nb);
    for (int i = 0 ; i < 7; i++) UNPACK32(ctx->h[i], &digest[i << 2]);
}
...
{
    uint64_t tot_len = ctx->tot_len + ctx->len;
    uint64_t block_nb = (1 + ((SHA256_BLOCK_SIZE - 9) < (ctx->len % SHA256_BLOCK_SIZE)));
    uint64_t pm_len = block_nb << 6;
    ...
    sha256_transf(ctx, ctx->block, block_nb);
    for (int i = 0 ; i < 8; i++) UNPACK32(ctx->h[i], &digest[i << 2]);
}
...
{
    for (int i = 0; i < 8; i++) ctx->h[i] = sha384_h0[i];
    ctx->len = 0; ctx->tot_len = 0;
}
...
{
    uint64_t tot_len = ctx->tot_len + ctx->len;
    uint64_t block_nb = (1 + ((SHA384_BLOCK_SIZE - 17) < (ctx->len % SHA384_BLOCK_SIZE)));
    uint64_t pm_len = block_nb << 7;
    ...
    for (int i = 0 ; i < 6; i++) UNPACK64(ctx->h[i], &digest[i << 3]);
}
...
{
    for (int i = 0; i < 8; i++) ctx->h[i] = sha512_h0[i];
    ctx->len = 0; ctx->tot_len = 0;
}
...
{
    uint64_t tot_len = ctx->tot_len + ctx->len;
    uint64_t block_nb = 1 + ((SHA512_BLOCK_SIZE - 17) < (ctx->len % SHA512_BLOCK_SIZE));
    uint64_t pm_len = block_nb << 7;
    ...
    sha512_transf(ctx, ctx->block, block_nb);
    for (int i = 0 ; i < 8; i++) UNPACK64(ctx->h[i], &digest[i << 3]);
}
